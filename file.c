...
static void print_target_dm(unsigned long pid, const char *name, unsigned long long count, int percentage)
{
	printk("cpu[%d] called pxt4_file_write_iter() %lld times (%d%%)\n", current->cpu, count, percentage);
}

DEFINE_DS_MONITORING(thread_dm, get_thread_idx, get_thread_name, print_target_dm);
//DEFINE_DS_MONITORING(thread_dm, get_thread_idx, get_thread_name, print_simple);
